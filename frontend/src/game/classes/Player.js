...
class Player {
  constructor({ x, y, size, velocity = { x: 0, y: 0 } }) {
    ...
    // Set default sprite and facing direction
  ...
  draw(c) {
    ...
    // Optional: Keep debug rectangle (comment out these lines to remove)
    ...
    c.drawImage(
      ...
      // Source: Extract the current frame from the sprite sheet
      this.currentSprite.x,
      this.currentSprite.y + this.currentSprite.height * this.currentFrame,
      this.currentSprite.width,
      this.currentSprite.height,
      // Destination: Draw on the canvas
      this.x,
      this.y,
      this.width,
      this.height
  ...
  update(deltaTime, collisionBlocks) {
    ...
    const intervalToGoToNextFrame = 0.15 // 150ms between frames
    ...
    // Update center position for camera/collision calculations
    this.center = {
      x: this.x + this.width / 2,
      y: this.y + this.height / 2,
    }
  ...
  handleInput(keys) {
    ...
    if (keys.d.pressed) {
      ...
      // When not moving, show idle frame (first frame of current direction)
      this.currentFrame = 0
    }
  }
